    ...
    <footer className="bg-surface border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 py-14">
        <div className="grid md:grid-cols-3 gap-10 mb-12">
          {/* Brand */}
          <div>
            <img src={logoImg} alt="Agape Arts Production" className="h-12 w-auto mb-4 object-contain" />
            <p className="font-sans text-white/50 text-sm leading-relaxed max-w-xs">
              Ghana's Premier Theatre Company — crafting unforgettable cultural experiences
              that transform communities and redefine African theatre.
            </p>
            <div className="flex gap-3 mt-6">
              {SOCIALS.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="w-9 h-9 rounded-sm border border-white/15 flex items-center justify-center text-white/40 hover:text-gold hover:border-gold/40 transition-all duration-300 hover:-translate-y-0.5"
                >
                  {s.icon}
                </a>
              ))}
          ...
          {/* Links */}
          <div>
            <h4 className="font-serif text-white font-bold mb-5">Quick Links</h4>
              ...
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <a
                    ...
                    className="font-sans text-white/50 text-sm hover:text-gold transition-colors duration-300"
                  >
                    {link.label}
                  </a>
                </li>
          ...
          {/* Contact */}
          <div>
            <h4 className="font-serif text-white font-bold mb-5">Contact</h4>
            <div className="space-y-3 font-sans text-sm text-white/50">
              <p>One Castle Road, Accra — Ghana</p>
              <p>+233 500 543 534</p>
              <p>agape4play@gmail.com</p>
            </div>
            <div className="mt-6">
              <a
                href="#contact"
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="font-sans text-white/30 text-xs">
            © 2026 Agape Arts Production. All rights reserved.
          </p>
          <div className="flex gap-6">
            {['Services', 'Gallery', 'FAQ', 'Contact'].map((l) => (
              <a
                key={l}
                href={`#${l.toLowerCase()}`}
                onClick={(e) => handleNav(e, `#${l.toLowerCase()}`)}
                className="font-sans text-white/30 text-xs hover:text-gold transition-colors duration-300"
              >
                {l}
              </a>
            ))}
